...
- **인터랙티브 터미널 히어로** — 방문자가 실제로 명령어를 칠 수 있어요. `help` · `whoami` · `cat profile.txt` · `about` · `work` · `contact` · `clear`. 정보 명령은 결과를 출력하고, 섹션 명령(`about`/`work`/`contact`)은 해당 섹션으로 부드럽게 스크롤됩니다.
